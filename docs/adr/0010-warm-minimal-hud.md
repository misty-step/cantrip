...
The HUD renders one fixed 360×40 borderless capsule, top-centre, on the
...
- **Motion**: the only continuous motion is a localized breathing pulse on
  the working glyphs (alpha 0.8–1.0, scale 0.96–1.0) — never a sweep,
  rotation, or length change. State changes crossfade over ~260 ms with a
  pop-in; outcome flashes are static compositions for ~2.5 s.
...
- **Reduced motion**: read once at startup from
  ...
  disabled, the pulse freezes, the recording glyph is a plain static dot,
  and state changes swap instantly.
- **Determinate progress**: the "Warm Minimal" determinate cluster (five
  ...
  A future `progress { stage, fraction }` daemon event gates it.

## Why not alternatives

- Width-following fill / rotating arcs (ADR 0006 style): fake progress; the
  daemon has no fraction to display.
