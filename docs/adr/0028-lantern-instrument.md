...
- **Presentation shares the field's clock.** Colour, rim and caption cross-
  fade over the same 400 ms settle as the field; success settles centre-out
  inside that settle (never left to right, which would read as progress); the
  capsule fades in over 120 ms. Reduced motion is instant. Lost status freezes
  presentation exactly as it freezes cells.
