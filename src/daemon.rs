...
fn spawn_worker(model_dir: PathBuf, warm: bool) -> WorkerChannels {
    ...
    let worker = thread::spawn(move || {
        ...
        while let Ok(wav) = job_rx.recv() {
            ...
            let transcription = transcribe_one(&mut transcriber, &model_dir, &wav)
                .map_err(|error| format!("{error:#}"));
            // Cleanup failure must never cost the user their words: the WAV
            // sits in the per-user tmpfs runtime dir, so log and move on.
            if let Err(cleanup_error) = remove_recording(&wav) {
                tracing::warn!("[Daemon] recording cleanup failed: {cleanup_error:#}");
            }
...
fn remove_recording(wav: &Path) -> Result<()> {
    match fs::remove_file(wav) {
        Ok(()) => Ok(()),
        Err(error) if error.kind() == ErrorKind::NotFound => Ok(()),
        Err(error) => Err(error).with_context(|| format!("removing recording {}", wav.display())),
    }
}
...
fn handle_connection(
    ...
    let bytes = reader
        ...
        .context("reading client command")?;
    if bytes == 0 {
        return Ok(());
    }
    ...
    stream.flush().context("flushing daemon reply")?;
    Ok(())
}
...
fn stop_recording(state: &mut State, job_tx: &Sender<PathBuf>) -> Reply {
    ...
    let State::Recording { recorder, started } = previous else {
        *state = previous;
        return Reply {
            ok: false,
            state: state.name().to_owned(),
